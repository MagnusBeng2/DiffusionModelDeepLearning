...
class CIFARFromNPZ(Dataset):
    def __init__(self, npz_path):
        data = np.load(npz_path)
        self.images = torch.tensor(data['images'], dtype=torch.float32)  # shape: (N, 3, 32, 32)
...
def train_ddpm(batch_size, epochs, learning_rate, timesteps, beta_schedule, enable_cfg=False):
    ...
    dataset = CIFARFromNPZ("data/processed/cifar_train.npz")
    ...
    model = UNet(time_emb_dim=256, num_classes=10).to(device)
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 250 == 0:
            checkpoint_path = f"models/ddpm_cifar_epoch_{epoch+1}.pth"
            torch.save(model.state_dict(), checkpoint_path)
            print(f"Saved model checkpoint to {checkpoint_path}")

            ema_path = f"models/ema_ddpm_cifar_epoch_{epoch+1}.pth"
            torch.save(ema_model.state_dict(), ema_path)
            print(f"Saved EMA model checkpoint to {ema_path}")
    ...
    torch.save(model.state_dict(), "models/ddpm_cifar_final.pth")
    torch.save(ema_model.state_dict(), "models/ema_ddpm_cifar_final.pth")
