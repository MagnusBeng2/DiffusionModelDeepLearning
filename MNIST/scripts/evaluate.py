...
def evaluate_model(model_path, num_samples, timesteps, use_random_noise=True, save_results=True):
    ...
    if use_random_noise:
        ...
        noisy_images = torch.randn_like(real_images)
    else:
        print("Using slightly noisy real images...")
        noisy_images = real_images.clone()

    random_labels = torch.randint(0, 10, (noisy_images.size(0),), device=device)
    generated_images = reverse_diffusion(
        model, device, noisy_images, timesteps,
    ...
    real_features = get_features(real_incep, inception)
    gen_features = get_features(gen_incep, inception)
